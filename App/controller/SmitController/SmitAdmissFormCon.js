...
const { updateSpecificData } = require('../../helper/index');
...
UpdateId = async (req, res) => {
    const { rollNo, generatedId } = req.body;
    console.log(req.body)
    let searchQuery = { "rollNo": rollNo }
    let updateQuery = {
        "generatedId": generatedId
    }
    const updatedResult = await updateSpecificData('smitAddForm', searchQuery, updateQuery);
    return res.status(200).json({
...
findData = async (req, res) => {
    const { city, course, batchName } = req.body;
    console.log(req.body, req.body)
    try {
        var query = { cityName: city, courseName: course, batchName };
        const result = await SmitAdmissFormModel.find(query)
        return res.status(200).json({
