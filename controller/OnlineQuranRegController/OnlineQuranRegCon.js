const OnlineQuranRegModel = require("../../model/OnlineQuranReg/OnlineQuranRegModel");
const cloudinary = require("../../cloudinary");
const checkField = require("../../FieldValidation/checkField");

OnlineQuranRegAdd = async (req, res) => {
  const {
    stdName,
    fthName,
    stnAge,
  ...
  if (!stdName) {
    ...
    try {
      ...
      try {
        const result = new OnlineQuranRegModel({
...
OnlineQuranRegGet = async (req, res) => {
  // checkField;
  const result = await OnlineQuranRegModel.find();
...
OnlineQuranRegDel = async (req, res, next) => {
  const { id } = req.body;
  if (!id) {
    return res.status(400).json(checkField("Student ID"));
  } else {
    let resu = await OnlineQuranRegModel.findById(id);
    try {
      if (!resu) {
        ...
        await OnlineQuranRegModel.findByIdAndDelete(id, function (err, result) {
          if (err) {
