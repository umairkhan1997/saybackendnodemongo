const QuranAppIosSchema = require("../../model/QuranAppTaf/QuranAppIos");
const cloudinary = require("../../cloudinary");
const checkField = require("../../FieldValidation/checkField");

QuranAppIosAdd = async (req, res) => {
...
QuranAppIosGet = async (req, res) => {
  // checkField;
  const result = await QuranAppIosSchema.find();
  res.status(200).json({
    success: true,
    data: result,
  });
...
QuranAppIosDel = async (req, res, next) => {
  const { id } = req.body;
  if (!id) {
    return res.status(400).json(checkField("App ID"));
  } else {
    let resu = await QuranAppIosSchema.findById(id);
    try {
      if (!resu) {
        ...
        await QuranAppIosSchema.findByIdAndDelete(id, function (err, result) {
          if (err) {
